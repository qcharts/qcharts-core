...
export class Tooltip extends BasePlugin {
  ...
  beforeRender() {
    ...
    this.dataset.on('hover:data', throttle(d => {
      if (!d) {
        ...
        if (hide) {
          var self = this; // 如果第一次出现，直接出现到当前位置
          ...
          this.$group.on('afterdraw', function fixPos() {
            ...
            setTimeout(() => {
              self.setState({
                pos: refixTooltipPosition(x, y, width, height, chartWidth, chartHieght)
              });
            }, 300);
  ...
  render() {
    ...
    return qcharts.h(Group, _extends({}, _objectSpread(_objectSpread(_objectSpread({
      clipOverflow: false,
