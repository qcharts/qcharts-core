...
var isNull = obj => obj === undefined || obj === null;

var diffVNodeChildren = (oldVNode, newVNode) => {
  var patches = []; // 新旧节点的子节点长度可能不一样，但是进行比对的次数应当为最长子节点数目

  var len = Math.max(oldVNode.children.length, newVNode.children.length);
...
var diffVNodeAttrs = (oldVNode, newVNode) => {
  var patches = [];
  var oldVNodeAttrs = oldVNode.attrs;
  var newVNodeAttrs = newVNode.attrs;
  var attrs = Object.assign({}, oldVNodeAttrs, newVNodeAttrs);
  patches = attrs;
  return patches;
};
/**
...
var isDiffrentVNode = (oldVNode, newVNode) => {
