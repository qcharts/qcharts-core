...
var getPathStyle = (pointType, scale) => {
  ...
  return scaleArray ? _objectSpread(_objectSpread({}, style), {}, {
    scale: scaleArray
  }) : _objectSpread({}, style);
};
/**
...
export function getSymbolAndStyle(style, hStyle) {
  ...
  var mergeNormalStyle = _objectSpread(_objectSpread({}, style), normalStyle);

  var mergeHoverStyle = _objectSpread(_objectSpread({}, hStyle), hoverStyle); // 非图片样式的删除 size属性，避免bug
