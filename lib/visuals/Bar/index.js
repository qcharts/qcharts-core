...
export class Bar extends BaseVisual {
  ...
  showTooltip(evt, data) {
    evt.data = data;
    this.dataset.hoverData(_objectSpread(_objectSpread({}, evt), {}, {
      data: data
    }));
  }
  ...
  render(data) {
    return qcharts.h(Group, {
      ...
      return qcharts.h(Sprite, _extends({
        ref: el => this.ref('groups', el)
      }, pillar, normalState, {
        hoverState: Object.assign({
          opacity: 0.05
        }, this.style('backgroundpillar:hover')(pillar, pillar.dataOrigin, pillar.index)),
        onMouseenter: (_, el) => !this.attr('mouseDisabled') && el.attr('state', 'hover') && this.showTooltip(_, pillar.rects),
        onMousemove: (evt, el) => {
