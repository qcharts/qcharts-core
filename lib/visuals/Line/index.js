...
export class Line extends BaseVisual {
  ...
  setHoverIndex(ind, options) {
    ...
    if ($guideline && tarIndex !== this.__guidelineIndex) {
      ...
      this.renderData.forEach(line => {
        line.data.forEach((data, i) => {
          if (i === tarIndex && line.disabled !== true) {
            hoverData.push(_objectSpread(_objectSpread({}, data.dataOrigin), {}, {
              color: data.color,
      ...
      if (this.attr('stack') === true) {
        hoverData.reverse();
      } else {// hoverData.sort((a, b) => {
        //   return b._value - a._value
        // })
      }

      this.dataset.hoverData(_objectSpread(_objectSpread({}, options), {}, {
        data: hoverData
      }));
  ...
  bgMove(evt, el) {
    ...
    if (!this.isMoving) {
      var tooltip = this.chart.plugins.filter(d => d instanceof Tooltip)[0];
